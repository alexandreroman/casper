...
> **The per-workspace `+/−` diff summary is dropped** (decision, 2026-07-06) — it
> is no longer planned. The branch-vs-merge-base divergence badge on each
...
- `Workspace` gains `kind: primary | linked` and `baseBranch`.
  `LayoutNode`/`Surface`/`Todo`/`AgentState` unchanged. (A derived `diffStat` was
  designed for the dropped diff summary; it was never built.)
...
(`+<insertions>` green / `−<deletions>` red, line counts only, hidden when empty).
It is superseded by the title-bar working-tree-vs-HEAD summary, which already
ships.
