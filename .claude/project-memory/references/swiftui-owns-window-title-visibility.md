...
Keep the window title out of the title bar with
`.toolbar(removing: .title)` (`ToolbarDefaultItemKind.title`, macOS 15+) on the
root view. Setting `window.titleVisibility = .hidden` from AppKit is at best a
defensive fallback, never the mechanism to rely on.
...
SwiftUI update then stays on screen until the next `NSWindow.didUpdateNotification`
— in practice until the user moves the mouse. `.toolbar(removing: .title)` makes
SwiftUI report `titleVisibility == .hidden` from the very first frame, and it
never flips back.
