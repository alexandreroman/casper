...
`.macOS(.v15)` and `CLAUDE.md`). At that target `CGWindowListCreateImage`
is **obsoleted** — a hard compile error, not just a deprecation — so it cannot be
used. A plain view/bitmap capture can't read a Metal-rendered (libghostty) window's
pixels, so ScreenCaptureKit is the only correct replacement. The migration made
`screenshot(window:to:)` `async`, which ripples `async` up through its private
