...
directory to `PATH`** in every terminal surface's environment. The generated
`.claude/settings.local.json` keeps the *relative* hook command `casper hooks
feed`, which therefore resolves only within Casper's terminals — nowhere else on
the user's system.
...
(`Bundle.main.executableURL`'s parent) and the inherited `PATH`. Trade-off: if a
user runs `claude` on that worktree from a NON-Casper terminal, the hook command
is not found — acceptable, since that is outside Casper's scope. See
