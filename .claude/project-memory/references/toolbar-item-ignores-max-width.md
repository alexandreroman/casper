...
SwiftUI measures a `ToolbarItem`'s content under an **unspecified** proposal, and
a flexible `.frame(maxWidth: budget)` answers such a proposal by proposing
nothing downward. A `ViewThatFits` behind that frame therefore always selects its
**widest** candidate, whatever the budget says. Measured directly: candidates of
300 / 200 / 100 pt behind `.frame(maxWidth: 199)` still select the 300 pt one.
