...
The `casper` CLI is **not** installed globally: there is no
`~/.local/bin/casper` shim. `casper` only needs to be reachable inside terminals
Casper opens, so Casper **prepends its own binary's directory to `PATH`** in
every terminal surface's environment. This is why a bare
`casper status set working` (or any domain command — see
[[domain-cli-control-channel]]) resolves inside a Casper terminal but nowhere
else on the system.
...
scripts use `"$GHOSTTY_BIN_DIR/casper"` — libghostty exports the bin directory
of the bundle that opened the terminal, so the script always drives the instance
