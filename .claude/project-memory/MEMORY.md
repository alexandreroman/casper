...
- [MainActor isolated delegate conformance](references/mainactor-isolated-delegate-conformance.md) — a @MainActor class conforming to a non-@MainActor Cocoa delegate (UNUserNotificationCenterDelegate) needs `@MainActor` on the conformance; @MainActor-annotated protocols (WKNavigationDelegate) don't
...
- [libghostty macOS config dir is bundle-id scoped](references/ghostty-config-dir-bundle-id.md) — bundled Casper.app misses the user's Ghostty config (empty com.github.alexandreroman.casper dir) → vanilla gray; hence the baked-in default theme
...
- [SwiftUI owns the main menu](references/swiftui-mainmenu-miniaturize-resync.md) — menu bar is `.commands`; empty Format/Help stubs stripped on will+didUpdate; the `.commands` body must not observe volatile focus/spaces (else SwiftUI re-asserts the menu & flickers the stubs) → edge-triggered enable flags + always-enabled Split
...
- [Browser console/wait/reload + retain cycle](references/browser-console-capture.md) — console/wait/reload verbs; WeakScriptMessageHandler's weak back-ref breaks WKUserContentController's strong handler retain (no deinit needed)
