...
- [Diff-view refresh hang (animated fixed; non-animated variant open)](references/diff-view-refresh-hang.md) — animated LazyVStack-relayout cause fixed (dedup + disablesAnimations), but build 766 still hangs via non-animated StackLayout/_FlexFrameLayout/StyledText recursion; bounded via lineLimit + fixedSize (unverified live)
- [Main-thread hang watchdog and dump exploitation](references/hang-dump-watchdog.md) — release-active watchdog auto-samples on a >2s main-thread block to ~/Library/Logs/Casper/hang-*.txt; how to read dumps + os_log marker, env tuning, manual capture (temporary, revert once fixed)
