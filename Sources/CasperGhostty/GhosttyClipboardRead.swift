...
enum GhosttyClipboardRead {
    ...
    /// (`ClipboardConfirmationView` with `Ghostty.ClipboardRequest.osc_52_read`) rendered as
    /// an `NSAlert` — same wording, same caution framing, same preview of the content.
    ///
    /// One deliberate deviation, the one the write prompt documents as well: Ghostty binds
    /// Return to "Allow". Casper leaves the consequential button off the Return key, as its
    /// other destructive alerts do, because this prompt is raised by output the user never
    /// asked for and can appear under their hands mid-typing — a stray Return must not hand
    /// a password or a token to whatever printed the escape sequence.
    private static func presentConfirmation(_ text: String) -> Bool {
        let alert = NSAlert()
        alert.alertStyle = .warning
        alert.messageText = "An application is attempting to read from the clipboard."
...
enum GhosttyClipboardPrompt {
    ...
    static func contentPreview(_ text: String) -> NSView {
