...
/// Upper bound on a single control frame payload (8 MB), applied symmetrically to
...
private let maxControlFrameBytes = 8 * 1024 * 1024
...
private let controlReplyLingerTimeout: TimeInterval = 15
...
/// Frames `payload` for the control channel: a 4-byte big-endian length prefix
...
public final class ControlSocketServer: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "casper.control-socket.server")
    ...
    public func start() throws {
        ...
        if bound.wait(timeout: .now() + bindTimeout) == .timedOut {
            listener.cancel()
            self.listener = nil
            throw ControlSocketError(reason: "timed out binding control socket at \(socketPath)")
        }
    ...
    private func receive(on connection: NWConnection) {
        ...
        readExactly(4, on: connection) { [weak self] header in
            ...
            guard length > 0, length <= UInt32(maxControlFrameBytes) else {
    ...
    private func dispatch(_ payload: Data, on connection: NWConnection) {
        guard let command = try? JSONDecoder().decode(ControlCommand.self, from: payload) else {
    ...
    private func reply(_ response: ControlResponse, on connection: NWConnection) {
    ...
    private func lingerUntilClientCloses(_ connection: NWConnection) {
        queue.asyncAfter(deadline: .now() + controlReplyLingerTimeout) { [weak self] in
    ...
    private func waitForClientClose(on connection: NWConnection) {
        connection.receive(minimumIncompleteLength: 1, maximumLength: maxControlFrameBytes) {
...
public enum ControlSocketClient {
    ...
    /// Sends `command` and returns the decoded `ControlResponse`.
    ///
    /// Set `retriable` to true ONLY for idempotent verbs. A transport-level
    /// failure (`ControlSocketError`) then triggers a bounded retry over a fresh
    ...
    /// arrives. A decoded `ControlResponse` — even `ok: false` — is a real
    /// answer and is returned immediately, never retried, so a mutating verb is
    /// never applied twice.
    public static func send(
        ...
        var lastError: ControlSocketError?
        ...
        // Only reached when every attempt threw a transport `ControlSocketError`.
        throw lastError ?? ControlSocketError(reason: "no response from \(socketPath)")
    }

    /// One request/response exchange over a single fresh connection. Throws a
    /// `ControlSocketError` on any transport failure; returns whatever
    /// `ControlResponse` the server delivers, including an `ok: false` one.
    private static func sendOnce(
        _ command: ControlCommand, toSocketAt socketPath: String, timeout: TimeInterval
    ) throws -> ControlResponse {
        guard FileManager.default.fileExists(atPath: socketPath) else {
            throw ControlSocketError(reason: "no socket at \(socketPath) (is the GUI running?)")
        }

        let payload = try JSONEncoder().encode(command)
        guard payload.count <= maxControlFrameBytes else {
            throw ControlSocketError(reason: "request too large: \(payload.count) bytes")
        }
        ...
        let queue = DispatchQueue(label: "casper.control-socket.client")
        let done = DispatchSemaphore(value: 0)
        let result = OSAllocatedUnfairLock<Result<ControlResponse, Error>?>(initialState: nil)
        ...
        @Sendable func finish(_ value: Result<ControlResponse, Error>) {
        ...
        @Sendable func receiveResponse() {
            readExactly(4, on: connection) { header in
                guard let header else {
                    finish(.failure(ControlSocketError(reason: "no response from \(socketPath)")))
                    connection.cancel()
                    return
                }
                let length = decodeLength(header)
                guard length > 0, length <= UInt32(maxControlFrameBytes) else {
                    finish(.failure(ControlSocketError(reason: "invalid response length: \(length) bytes")))
                    connection.cancel()
                    return
                }
                readExactly(Int(length), on: connection) { payload in
                    guard let payload else {
                        finish(.failure(ControlSocketError(reason: "truncated response from \(socketPath)")))
                        connection.cancel()
                        return
                    }
                    if let response = try? JSONDecoder().decode(ControlResponse.self, from: payload) {
                        finish(.success(response))
                    } else {
                        finish(.failure(ControlSocketError(reason: "undecodable response")))
                    }
                    connection.cancel()
                }
            }
            ...
            case .ready:
                ...
                connection.send(
                    ...
                    completion: .contentProcessed { error in
                        if let error {
                            finish(.failure(ControlSocketError(reason: "\(error)")))
                            connection.cancel()
                        }
                    })
                receiveResponse()
        ...
        if done.wait(timeout: .now() + timeout) == .timedOut {
            connection.cancel()
            throw ControlSocketError(reason: "timed out talking to \(socketPath)")
        }
        switch result.withLock({ $0 }) {
        case .success(let response): return response
        case .failure(let error): throw error
        case .none: throw ControlSocketError(reason: "no response from \(socketPath)")
        }
    }
}
